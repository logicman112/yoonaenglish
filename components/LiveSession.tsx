  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setTranscripts(prev => [...prev, { 
        role: 'model', 
        text: finalChatText || "여보, 방금 한 말 다시 한 번만 말해줄래? (Could you say that again?)", 
        timestamp: Date.now(),
    ...
    <div className="max-w-4xl mx-auto flex flex-col h-[calc(100vh-14rem)] md:h-[850px] animate-in zoom-in-95 duration-700">
      <div className="flex justify-between items-center mb-6 px-4">
        ...
        <button 
          ...
          className={`px-6 py-3 ${isGrumpy ? 'bg-slate-700' : 'bg-rose-500'} text-white rounded-2xl font-black text-sm shadow-xl transition-all flex items-center gap-2 disabled:opacity-50`}
        >
      ...
      <div className="flex-1 bg-white rounded-[3.5rem] border border-slate-100 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col relative">
        ...
        <form onSubmit={handleSend} className="p-8 bg-white border-t border-slate-50 flex gap-4 items-center">
          ...
          />
          <button type="submit" disabled={!inputValue.trim() || isLoading} className={`w-16 h-16 bg-gradient-to-tr ${isGrumpy ? 'from-slate-700 to-slate-900 shadow-slate-200' : 'from-pink-500 to-rose-600 shadow-pink-200'} hover:scale-105 disabled:bg-slate-200 text-white rounded-3xl shadow-2xl flex items-center justify-center transition-all active:scale-95`}>
            <svg className="w-8 h-8 rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
          </button>
        </form>
      </div>
    </div>
